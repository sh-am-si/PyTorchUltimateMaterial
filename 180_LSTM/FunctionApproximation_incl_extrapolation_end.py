...
y = [
    np.cos(X[i] * np.pi / 180) * (1 + i / num_points) + (np.random.randn(1) * 0.1)[0]
...
X_extrapolate = torch.cat(
    (
        X_test_torch.unsqueeze(2),
        torch.zeros(X_test_torch.shape[0], num_future_steps, 1),
    ),
    dim=1,
)
...
with torch.no_grad():
    ...
    y_pred_extrapolated = (
        X_extrapolate[:, -num_future_steps:, :].squeeze(2).cpu().numpy()
    )
...
sns.lineplot(
    x=np.linspace(
        0,
        len(y_pred_extrapolated.flatten("F")) - 1,
        num=len(y_pred_extrapolated.flatten("F")),
    ),
    y=y_pred_extrapolated.flatten("F"),
    label="Extrapolated",
    color="blue",
)
